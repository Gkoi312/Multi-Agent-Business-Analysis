...
- ✅ 387 automated tests passing (`backend/tests/`)
...
- ✅ Fixed a wiring bug that made relevance scoring a no-op on every real run: `InterviewState` and the `Send`-based fan-out payload never carried `company_name`/`focus` through to `ToolContext`, so `RelevanceScoreStage` always took its "no target" early exit (flat 0.5 score, nothing filtered). Also closed several CJK blind spots in the cleaning pipeline found while tracing it end-to-end: keyword-density scoring, date extraction, and content fingerprinting all used whitespace tokenization, which collapses a whole Chinese paragraph into a single "word"; made each CJK-aware (character-level tokenization), added Chinese date patterns, and seeded the previously-empty spam-domain blocklist
- ✅ Made `evidence_quality` mechanical instead of LLM self-reported — derived from the count of distinct source IDs (2+ → high, 1 → medium, 0 → low) via one shared helper used by both the compressor and the fact reconciler. Also fixed source-ID dedup across rounds: a URL resurfacing in a later round now reuses its existing `source_id` instead of minting a new one, which had been artificially inflating independent-source counts and letting a single source pass as multi-source corroboration
- ✅ Removed dead tool-calling scaffolding (`ToolContextPruner` and the tool-call boundary-completion helpers in `HistoryCompactor`/`RunningSummaryManager`) — no domain in this repo does LangChain-style tool-calling yet (search runs as a plain function call), so this machinery was never exercised in production; left comments at each site for when a future ReAct-style domain needs it back. Fixed a real bug uncovered in the same pass: `HistoryCompactor`'s round-boundary splitting could separate a question from its answer at an odd split index
- ✅ Removed the unused model-driven fact-reconciliation path (`FactReconciler.reconcile_from_model_output`) — `WorkingMemory` only ever calls the code-driven SPDV `reconcile()`. Made SPDV fact matching CJK-aware: the old word-split tokenizer collapsed an entire Chinese sentence into one token, so near-duplicate Chinese facts never matched via the Jaccard fallback
- ✅ Fixed an `InvalidUpdateError` on the interview fan-out merge by adding a `keep_latest` reducer to every scalar field in `ResearchGraphState` — parallel interview branches writing back to shared state need an explicit merge policy per field, not just for the list-typed ones
- ✅ Fixed premature interview termination: `ask_question` was being shown the full accumulated fact list instead of just the remaining knowledge gaps, which let the model conclude research was complete too early
- ✅ History compaction now surfaces its own summarization LLM call's token cost as `llm_metrics` (previously invisible to token accounting), and context assembly no longer double-bills tokens: messages already folded into the running summary are excluded from raw recent messages, and sources already cited in an earlier round are no longer re-embedded in full on repeat mentions
- ✅ Added an A/B compression-comparison script (`backend/scripts/run_compression_comparison.py`) that runs the same task with compression fully enabled vs. fully disabled — including `compact_history`, not just the per-turn `compress` node, so the "no compression" arm is a clean baseline rather than one that still benefits from cutoff-pruning — to measure the real token/cost delta
